...
Infelizmente tal medida não existe ao considerar $\mcal{P}(\mathbb{R})$ como domínio. Primeiro construiremos uma $\sigma$-álgebra em que (i), (ii) e (iii) serão satisfeitas.
...
\begin{definition*}{}
Para um conjunto $A \in \mcal{P}(\mathbb{R})$, definimos sua medida exterior como
...
Segue imediatamente da definição que $m^*(A) \ge 0, \forall A \in \mcal{P}(\mathbb{R})$, $m^*(\emptyset) = 0$ e que se $A \subseteq B$, então $m^*(A) \le m^*(B)$, uma vez que toda cobertura por intervalos de $B$ também cobrirá $A$.
...
\begin{definition*}{}
Dizemos que $E \in \mcal{P}(\mathbb{R})$ é (Lebesgue) mensurável se
$$ m^*(A) = m^*(A \cap E) + m^*(A \cap E^c), \quad \forall A \in \mcal{P}(\mathbb{R}) $$
...
Além disso, note que $A = (A \cap E) \cup (A \cap E^c), \forall A \in \mcal{P}(\mathbb{R})$, e portanto,
$$ m^*(A) \le m^*(A \cap E) + m^*(A \cap E^c). $$
Assim, para verificar se $E$ é mensurável, basta apenas mostrar que
$$ m^*(A) \ge m^*(A \cap E) + m^*(A \cap E^c), \quad \forall A \in \mcal{P}(\mathbb{R}). $$
...
\begin{proof}[\textbf{Dem:}]
$$ m^*(A \cap \emptyset) + m^*(A \cap \emptyset^c) = m^*(\emptyset) + m^*(A) = m^*(A), \quad \forall A \in \mcal{P}(\mathbb{R}). $$
$$ m^*(A \cap \mathbb{R}) + m^*(A \cap \mathbb{R}^c) = m^*(A) + m^*(\emptyset) = m^*(A), \quad \forall A \in \mcal{P}(\mathbb{R}). $$
Como $(E^c)^c = E$, segue que se $E$ é mensurável, então
$$ m^*(A \cap E^c) + m^*(A \cap (E^c)^c) = m^*(A \cap E^c) + m^*(A \cap E) = m^*(A), \quad \forall A \in \mcal{P}(\mathbb{R}). $$
\end{proof}
...
\begin{proof}[\textbf{Dem:}]
Seja $A \in \mcal{P}(\mathbb{R})$. Como $A \cap E \subseteq E$, então $m^*(A \cap E) \le m^*(E) = 0 \implies m^*(A \cap E) = 0$.
...
\begin{proof}[\textbf{Dem:}]
Inicialmente, provaremos que se $E_1, E_2$ são mensuráveis, então $E_1 \cup E_2$ é mensurável. Seja $A \in \mcal{P}(\mathbb{R})$. Então
...
\begin{prop*}{7}
Seja $A \in \mcal{P}(\mathbb{R})$ e $\{E_k\}_{k=1}^n$ uma coleção finita de conjuntos disjuntos mensuráveis. Então
...
Assim, para $A \in \mcal{P}(\mathbb{R})$,
...
\bigline
\noindent
\textbf{Conclusão:} A coleção $\mcal{M} = \{E \in \mcal{P}(\mathbb{R}) ; E \text{ é Lebesgue mensurável}\}$ é uma $\sigma$-álgebra.
