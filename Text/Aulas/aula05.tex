...
Como $\mcal{M}$ é uma $\sigma$-álgebra, basta provar que $(a, +\infty) \in \mcal{M}$, 
 para todo $a \in \mathbb{R}$.

Seja $A \in \mcal{P}(\mathbb{R})$. Como $m^*(A) = m^*(A \setminus \{a\})$, podemos supor que $a \notin A$. Mostremos que
...
\noindent
\textbf{Conclusão:} $\mcal{B}(\mathbb{R}) \subseteq \mcal{M}$.
De fato, como $\mcal{B}(\mathbb{R}) = \sigma(\{(a, +\infty) ; a \in \mathbb{R}\}) \implies \mcal{B}(\mathbb{R}) \subseteq \mcal{M}$. 
...
\begin{proof}[\textbf{Dem:}]
Seja $E$ um conjunto mensurável e $y \in \mathbb{R}$. Assim, para $A \in \mcal{P}(\mathbb{R})$, temos
\begin{align*}
...
\begin{definition*}{}
A restrição da medida exterior para os conjuntos mensuráveis é chamada de medida de Lebesgue, $m: \mcal{M} \rightarrow \overline{\mathbb{R}}$, $m(E) := m^*(E)$.
\end{definition*}
...
\begin{prop*}{12}
O espaço de medida $(\mathbb{R}, \mcal{M}, m)$ é completo.
\end{prop*}{}

\begin{proof}[\textbf{Dem:}]
Seja $A \subseteq E$, com $E \in \mcal{M}$ e $m(E) = 0$. Assim, $m^*(E) = 0$, e como $A \subseteq E$, $m^*(A) \le m^*(E) = 0$. Logo $m^*(A) = 0$ e, pela Proposição 5, concluímos que $A \in \mcal{M}$.
\end{proof}
...
\begin{theorem*}{}
Seja $E \in \mcal{P}(\mathbb{R})$. São equivalentes:
\begin{itemize}
