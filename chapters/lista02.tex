...
\item Utilizando o exercício anterior, mostre que todo conjunto $E \in \mcal M$ 
  pode ser escrito na forma $A = F\cup Z$, em que $F \in \mcal B(\mbb R)$ e $Z \subset N$ 
    ...
    \begin{proof}[\textbf{Dem:}] Considere $E$ mensurável. Visto que existe $F_\sigma$
      denotado por $H$, com $H \subseteq E$, tal que $m^*(E\setminus H) = 0$, segue que 
      $C = E\setminus H$  é mensurável. 
      Daqui, para cada $k \in \mbb N$, considere abertos $U_k \supset C$, tais que 
      $m^*(U_k\setminus C) < \frac{1}{k}$. Note que, 
      \begin{align*}
        \frac{1}{k}>m^*(U_k\setminus C) = m^*(U_k) - m^*(C) = m^*(U_k).
      \end{align*}
      Com isso, $m^*(\bigcap U_k) = \mu_B(\cap U_k) < 1/k$ para todo $k$, ou 
      seja, $\mu_B(\bigcap U_k) = 0$. Mais ainda $D = \bigcap U_k$ é $G_\delta$
      e portanto elemento da $\sigma$-álgebra de 
      Borel. Desse modo, já que, 
      \begin{align*}
        H \cup C = H \cup E\setminus H = H \cup (E \cap H^C) = E,
      \end{align*}
      e, $C \subset U_k$ para todo $k \in \mbb N$, i.e, $C \subset \bigcap U_k = D$, 
      temos $E \in \overline{\mcal B(\mbb R)}$. Para mostrar que $m = \overline \mu_B$ em 
      $\mcal M$, considere $A \in \mcal M$. Já que existem $E \in \mbb R$ e $Z \subset 
      N \in \mcal Z$ tais que $A = E \cup Z$, então, 
      \begin{align*}
        \overline \mu_B(A) = \mu_B(E) = m^*(E) = m(E) \le m(E\cup Z) = m(A).
      \end{align*}
      Por outro lado,
      \begin{align*}
        m(A) = m(E \cup Z) = m^*(E \cup Z) \le m^*(E) + m^*(Z) = m^*(E) = \mu_B(E) = 
        \overline \mu_B(E \cup Z) = \overline \mu(A),
      \end{align*}
      como queríamos.
    \end{proof}
\end{enumerate}}
...
\end{align*}}
\begin{proof}[\textbf{Dem:}] Ora, da hipótese, segue que $E_1 \cup E_2$ e $E_1 \cap E_2$
  também são mensuráveis. Desse modo, 
  \begin{align*}
    m(E_1) = m(E_1 \cap E_2) + m(E_1 \cap E_2^C) = m(E_1 \cap E_2) 
    + m([E_1 \cap E_2^C]\cap (E_1 \cup E_2))
  \end{align*}
\end{proof}
